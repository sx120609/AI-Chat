...
管理员可在“支付”选项卡新增、编辑和删除多个 Coding Plan，并分别配置上架状态、名称、用户说明、人民币售价、每月额度，以及是否在套餐有效期内开放个人 API Key。套餐默认关闭；上架后，用户可在个人中心选择订阅。每次支付会开通一个自然月；已有效的套餐再次购买会顺延一个月，不会自动续费。

套餐额度会与用户原有的月订阅额度合并，并按现有的“月订阅优先、AI 点数补足”规则扣减。支付订单会保存购买时的套餐快照，因此之后修改套餐价格或额度不会影响已创建订单；套餐到期后，套餐部分额度和套餐授予的个人 API 权益会自动失效。
